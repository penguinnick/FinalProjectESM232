...
evolve_pop_wofftake = function(fertility, survivability, initialpop, nstep, offtake, extrinsicP=1, K=0, minHerd) {
  ...
  for (i in 1:(nclasses-1)) {
    leslie_matrix[i+1,i] = survivability[i]*extrinsicP
    lm.offtake[i,i] = (1-(survivability[i]*extrinsicP))*offtake
  ...
  offtake.str[,1]=initialpop*(1-survivability)*offtake
  for (i in 2:nstep) {
    ...
    # if (minHerd>0){
      # offtake.str[,i] = ifelse(total.pop[i-1]<=minHerd,(lm.offtake-(lm.offtake*offtake))%*%pop.structure[,i-1],lm.offtake%*%pop.structure[,i-1])
    # }
    ...
    # }
    # offtake.str[,i] =
    # pop.structure[,i] = pop.structure[,i-1]-offtake.str[,i]
    # culledpop=pop.structure[,i-1]-offtake.str[,i]
    pop.structure[,i] =  leslie_matrix %*% pop.structure[,i-1] #) - offtake.str[,i-1]
    offtake.str[,i] = lm.offtake%*%pop.structure[,i]
