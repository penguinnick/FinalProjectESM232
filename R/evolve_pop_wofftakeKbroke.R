...
evolve_pop_wofftake = function(fertility, survivability, initialpop, nstep, offtake, K=0, minHerd=NULL) {
  ...
  lm.911offtake = lm.offtake
  # lm.offtake[nclasses,nclasses] = (1-survivability[nclasses])*offtake[nclasses]
  ...
  for (i in 2:nstep) {
    ...
    # if(!is.null(minHerd)&&total.pop[i]<minHerd){
    #   adj.offtake=(total.pop-minHerd)/total.pop
    #   for(j in 1:(nclasses)){
    #     lm.911offtake[j,j] = lm.offtake[j,j]-adj.offtake
    #   }
    #   offtake.str[,i] = lm.911offtake%*%pop.structure[,i-1] #(leslie_matrix %*% pop.structure[,i-1]) - (pop.structure[,i-1]*adj.offtake)
    # } else { offtake.str[,i] = lm.offtake%*%pop.structure[,i-1] }
...
# adjust.offtake <- function(lm.911offtake, survivability, minHerd, total.pop)
